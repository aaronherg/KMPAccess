<div align="center" width="50">

  <img src="https://github.com/tu-usuario/README/blob/main/recursos/kmpaccess_logo.png?raw=true" alt="KMPAccess Logo" width="60%"/><br> 

  <p><strong>KMPAccess - Gestión de permisos multiplataforma con Kotlin</strong></p>

  <div>
    <a href="#" style="display: inline-block;">
      <img src="https://github.com/tu-usuario/README/blob/main/recursos/captura1.png?raw=true" alt="Captura 1" style="width: 30%; margin: 0 1%;">
    </a>
    <a href="#" style="display: inline-block;">
      <img src="https://github.com/tu-usuario/README/blob/main/recursos/captura2.png?raw=true" alt="Captura 2" style="width: 30%; margin: 0 1%;">
    </a>
    <a href="#" style="display: inline-block;">
      <img src="https://github.com/tu-usuario/README/blob/main/recursos/captura3.png?raw=true" alt="Captura 3" style="width: 30%; margin: 0 1%;">
    </a>
  </div>

</div>

<hr></hr>

<div align="center" width="50">
  <p><strong>🚀 FUNCIONALIDADES PRINCIPALES DE KMPAccess</strong></p>
</div>

<div align="start">

<div align="center"><p><strong>1. Gestión de permisos multiplataforma</strong></p></div>
<ul>
  <li>Solicita y maneja permisos comunes en Android e iOS.</li>
  <li>Gestión de estados: permitido, denegado, restringido.</li>
  <li>Lógica compartida en Kotlin Multiplatform para evitar duplicación de código.</li>
</ul>

<div align="center"><p><strong>2. Ejemplos interactivos</strong></p></div>
<ul>
  <li>Pantallas de prueba para visualizar el estado de los permisos en tiempo real.</li>
  <li>Flujos de permisos explicados paso a paso para aprendizaje.</li>
</ul>

<div align="center"><p><strong>3. Buenas prácticas y arquitectura</strong></p></div>
<ul>
  <li>Uso de coroutines para llamadas asíncronas.</li>
  <li>Separación clara entre lógica compartida y UI específica de cada plataforma.</li>
  <li>Proyecto modular listo para integrar en otros proyectos KMP.</li>
</ul>

<hr></hr>

<div align="center" width="50">
  <p><strong>👑 CONTRIBUCIÓN</strong></p>
  ¡Agradecemos cualquier contribución para mejorar KMPAccess!<br>
  Haz un fork, realiza tus cambios y envía un pull request.
</div>

<hr></hr>

<div align="center" width="50">
  <p><strong>📝 LICENCIA</strong></p>
  Este proyecto está bajo la licencia MIT.
</div>

<hr></hr>

<div align="center" width="50">
  <p><strong>👨‍🔧 SOPORTE TÉCNICO</strong></p>
  <a href="https://www.instagram.com/">
    <img src="https://github.com/tu-usuario/README/blob/main/recursos/instagram.png?raw=true" alt="Instagram" width="5%">
  </a>
  &nbsp;&nbsp;
  <a href="https://www.facebook.com/">
    <img src="https://github.com/tu-usuario/README/blob/main/recursos/facebook.png?raw=true" alt="Facebook" width="5%">
  </a>
  &nbsp;&nbsp;
  <a href="https://twitter.com/">
    <img src="https://github.com/tu-usuario/README/blob/main/recursos/twitter.png?raw=true" alt="Twitter" width="5%">
  </a>
</div>
